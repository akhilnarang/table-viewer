// GET /proxy?url=<https://...>
// The page first tries to get a file directly. It calls this proxy only when the remote site
// blocks cross-origin requests. Both the Worker (src/worker.js) and the Pages Function
// (functions/proxy.js) use this code.

const MAX_BYTES = 100 * 1024 * 1024;

// Hosts that the proxy must not fetch. Cloudflare blocks most of these already.
...
export async function proxy(request) {
  if (request.method !== "GET") return new Response("method not permitted", { status: 405 });
